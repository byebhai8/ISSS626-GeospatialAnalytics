...
```{r}
accidents32647 <- read_rds("data/rds/accidents_sf.rds")
```
...
Likewise, we can run the glimpse() function for both the thaiRoads and admBndrs simple feature data frames.
...
#### 4.2.3.1 Accidents data

We want to use the code chunk below to perform the following actions:

-   Narrow down study area to the provinces in Bangkok Metropolitan Region (BMR)
-   Create new columns for Season, Day of Week and Time of Day
-   Remove province_th & route columns since the values are not in English
-   Remove report_datetime column since we only want to focus on when the incident was reported (incident_datetime)

```{r}
#|eval: false
bmr <- c("Nakhon Pathom", "Pathum Thani", "Nonthaburi", "Samut Prakan", "Samut Sakhon", "Bangkok")
accidentsCU <- accidents32647 %>%
  ...
  select(`acc_code`, `province_en`, `incident_datetime`, `dayofweek`, `month`, `season`, `weather_condition`,
         `road_description`, `slope_description`, `agency`, `vehicle_type`, `presumed_cause`, `accident_type`,
         `number_of_vehicles_involved`, `number_of_fatalities`, `number_of_injuries`, `geometry`)
write_rds(accidentsCU, "data/rds/accidentsCU.rds")
```

```{r}
accidentsCU <- read_rds("data/rds/accidentsCU.rds")
accidentsCU
```

Reference was taken from the following link to determine the seasons for BMR.
...
-   Narrow down highway fields
-   Remove name & name_th columns since the values are not in English
-   Cast geometry field from MULTILINESTRING to LINESTRING

```{r}
#|eval: false
highways <- c("motorway", "primary", "secondary", "tertiary", "unclassified", "residential", "service", "track",
              "road")
thaiRoadsCU <- thaiRoads32647 %>%
  filter(highway %in% highways) %>%
  select(`name_en`, `highway`, `surface`, `smoothness`, `width`, `lanes`, `oneway`, `bridge`, `layer`, `source`,
         `osm_id`, `geometry`)
thaiRoadsCU <- st_cast(thaiRoadsCU, "LINESTRING")
write_rds(thaiRoadsCU, "data/rds/thaiRoadsCU.rds")
```

```{r}
thaiRoadsCU <- read_rds("data/rds/thaiRoadsCU.rds")
thaiRoadsCU
```

Reference was taken from the following link to highway fields in this data set.

Link: <https://wiki.openstreetmap.org/wiki/WikiProject_Thailand#Highway_classification>

#### 4.2.3.3 Boundary data
