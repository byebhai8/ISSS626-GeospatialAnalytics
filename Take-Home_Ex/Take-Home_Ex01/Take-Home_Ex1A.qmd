...
admBndpRaw has a total of 77 features and 14 fields, with geometry type of polygon, and it is in wgs84 coordinates system.

Note that there are 2 administrative levels in this data set - ADM0 for country, and ADM1 for province.
...
To recap - there are 2 administrative levels in this data set - ADM0 for country, and ADM1 for province.
...
Then, we continue with out basic plots.

```{r}
tm_shape(admBndpSF) +
  tm_polygons(col = "grey", alpha = 0.3) +
  tm_shape(thaiAccSF) +
  tm_dots(col = "province_en", palette = "magma", size = 0.1) +
  tm_layout(main.title = "Accidents by Province", main.title.position = "left", main.title.size = 1.2, 
            legend.position = c("left","top"), legend.outside = TRUE, frame = TRUE) +
  tm_scale_bar() +
  tm_grid(alpha = 0.2)
```
...
tm_shape(admBndpSF) +
  tm_polygons(col = "grey", alpha = 0.3) +
  tm_shape(thaiAccSF) +
  tm_dots(col = "month", palette = "plasma", size = 0.1) +
  tm_layout(main.title = "Accidents by Month", main.title.position = "left", main.title.size = 1.2, 
            legend.position = c("left","top"), legend.outside = TRUE, frame = TRUE) +
  tm_scale_bar() +
  tm_grid(alpha = 0.2)
```
...
```{r}
ggplot() +
  geom_sf(data = admBndpSF, aes(color = ADM1_EN), size = 1, linewidth = 1) +
  geom_sf(data = thaiAccSF, aes(color = month), size = 0.1) +
  theme_minimal() +
